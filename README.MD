...
To run the bash app, please run the following command:

```bash
$ ruby init.rb
```

Then, you can register the items, one per time, and then it will print the result once you decide not to add new products to your order. Only the items which use the the valid format will be registered.

**Valid format:** `[quantity of products] [imported] [product name] at [product price with 2 decimal cases]`

#### Examples:

`1 bottle of water at 0.50`

`1 imported bottle of wind at 10.99`

The word imported right before the product name is not required.

The product price field must have 2 decimal cases, you can use dot (1.00) or comma (1,00).

In addition, you can run the specs which is also testing the challenge's inputs, just running the following cmd:
